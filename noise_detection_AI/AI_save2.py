...
def get_wav(path):
    ...
    for file_name in glob.glob(path):
        ...
        print(len(melspecs[0]))
        print(len(melspec_1.astype(np.float16)))
        sys.exit()
...
for i in range(50):
    ...
    # 学習結果保存
    with open(save_path + 'model_' + str(i) + '.pickle', mode='wb') as f:
        pickle.dump(clf.predict, f, protocol = 2)

print(f"train_mean = {np.min(train_mean)}")

# 学習データ保存（前処理・かさ増し1）
with open(save_path + '3add_little_train_feature.joblib', mode='wb') as f:
